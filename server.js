...
class EndpointCatalog {
  ...
  retrieveBlueprint(httpMethod, urlPath) {
    ...
    for (const entry of this.dynamicRegistry) {
      if (entry.httpMethod === httpMethod && this._matchesDynamicPattern(urlPath, entry.urlPattern)) {
  ...
  getAllSignatures() {
    const sigs = Array.from(this.exactRegistry.keys()).map(s => s.replace('|||', ' '));
    this.dynamicRegistry.forEach(e => sigs.push(`${e.httpMethod} ${e.urlPattern}`));
    return sigs;
  }
}
...
class RequestLifecycle {
  ...
  _wireEvents() {
    ...
    this.bus.subscribe('lookup:endpoint', async (ctx) => {
      ctx.endpointBlueprint = this.catalog.retrieveBlueprint(ctx.httpMethod, ctx.urlPath);
    ...
    this.bus.subscribe('error:inject', async (ctx) => {
      ...
      if (blueprint.errorRate) {
        ...
        if (randomValue < blueprint.errorRate) {
          ...
          ctx.responseData = blueprint.errorResponse || { error: 'Simulated error' };
          ctx.abortPipeline = true;
...
class MockServer {
  ...
  _loadBlueprint() {
    ...
    console.log(`Blueprint loaded with ${blueprint.endpoints?.length || 0} endpoints`);
  }
  ...
  async _handleIncomingRequest(req, res) {
    ...
    const ctx = await this.lifecycle.processRequest(req.method, urlObject.pathname);
    ...
    const bodyContent = typeof ctx.responseData === 'object'
